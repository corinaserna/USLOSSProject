...
int doSendMessage(const char *funcDesc, MailboxPtr mBox, char *msg_ptr, int msg_size) {
...
/* ------------------------------------------------------------------------
 Name - waitDevice
 This routine will be used to synchronize with a device driver process in the next phase. For this phase, only waitDevice is implemented.
 Do a receive operation on the mailbox associated with the given unit of the device type. The device types are defined in usloss.h. The appropriate device mailbox is sent a message every time an interrupt is generated by the I/O device. waitDevice returns the device’s status register in *status.
 The clock is slightly different. Only send a message to the clock mail box after every 5th clock interrupt (roughly every 100,000 microseconds).
 Return values:
